...
from memory import PrioritizedReplayMemory, Transition, State
...
def build_state(lm, obs, infos, prev_obs=None, prev_acts=None):
    ...
    if prev_obs is None:
        return [State(lm.sent2ids(ob), lm.sent2ids(info['look']), lm.sent2ids(info['inv']),
                 lm.sent2ids(build_input_str(info['task_description'], '', '', ob, info['inv'], info['look'])))
                for ob, info in zip(obs, infos)]
    else:
        states = []
        for prev_ob, ob, info, act in zip(prev_obs, obs, infos, prev_acts):
            sent = build_input_str(info['task_description'], prev_ob + ' ', act + ' ', ob, info['inv'], info['look'])
